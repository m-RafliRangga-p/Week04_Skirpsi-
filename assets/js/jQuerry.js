...
const faqData = [
  ...
  function createFaqItem(faq) {
    ...
    const $question = $('<h3>')
      ...
      .html(faq.question); // Menggunakan .html agar tag <sup> berfungsi
  
    // Membuat elemen hr sebagai garis pemisah
    const $separator = $('<hr>')
      .addClass('faq-separator')
      .hide(); // Menyembunyikan separator di awal
  
    // Membuat elemen p untuk jawaban
    const $answer = $('<p>')
      .addClass('faq-answer')
      .text(faq.answer)
      .hide(); // Menyembunyikan jawaban di awal
...
$(document).ready(function() { // Memastikan bahwa script berjalan setelah halaman selesai dimuat.
  ...
  $('#searchBtn').on('click', function() {
    ...
    const filteredMentors = mentors.filter(function(mentor) {
      ...
      return mentor.title.toLowerCase().includes(searchQuery) || mentor.rating.includes(searchQuery);
    });
...
$(document).ready(function() {
  ...
  $('#searchBtn').on('click', function() {
    ...
    const filteredCourses = courses.filter(function(course) {
      return course.title.toLowerCase().includes(searchQuery) || course.price.includes(searchQuery);
    });
